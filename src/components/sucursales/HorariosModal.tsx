        ...
        <form id="horarios-form" onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto">
          <div className="grid grid-cols-2 gap-6 h-full">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-700">Día de la Semana</h3>
              <div className="grid grid-cols-4 gap-2">
                {DIAS_SEMANA.map(dia => {
                  const horario = horarios.find(h => h.diaSemana === dia.num);
                  const isSelected = diaSeleccionado === dia.num;
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-xl text-center transition-all ${
                        isSelected
                          ? 'bg-[#0490C8] text-white ring-2 ring-[#0490C8] ring-offset-2 shadow-md'
                          : horario?.abierto
                          ? 'bg-gray-100 text-gray-700 border-2 border-gray-300 hover:bg-gray-200'
                          : 'bg-gray-50 text-gray-500 border-2 border-gray-200 hover:bg-gray-100'
                      }`}
                    >
                      <div className="font-bold text-sm">{dia.letra}</div>
                      <div className="text-[10px] mt-0.5">{dia.nombre.substring(0, 3)}</div>
                    </button>
                  );
                })}
              </div>
